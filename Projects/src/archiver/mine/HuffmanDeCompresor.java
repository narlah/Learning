...
public final class HuffmanDeCompresor extends DeCompressor {
	...
	private void getFrequencesFromFile() throws IOException {
		BinaryStdIn reader = new BinaryStdIn(new BufferedInputStream(new FileInputStream(new File(inFile))));
		treeFromFile = new LinkedHashMap<Character, Integer>();
	...
	public void decode() throws IOException {
		...
		while (!reader.isEmpty()) {
			...
			tempNode = (tmp == true) ? tempNode.right : tempNode.left;
			if (tempNode.isLeaf()) {
	...
	public String toString() {
		return outFile + " -> " + "Huffman DE Compressor" + " -> " + inFile;
	}
}
